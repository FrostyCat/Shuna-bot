...
def coc_members(guild_id):
    ...
    if not role_id:
        db.close()
        flash("Set the clan member role in Settings first.", "danger")
        return redirect(url_for("coc_manager", guild_id=guild_id))
    ...
    role_members = [m for m in all_guild_members if role_id in m.get("roles", [])]
    ...
    for member in role_members:
        ...
        db_user = db.query(DiscordUser).filter_by(discord_id=disc_id).first()
        players = db_user.players if db_user else []

        if not players:
            no_account_rows.append({
                "discord_id": disc_id,
                "discord_username": discord_username,
                "linked": False,
            })
        ...
        for player in players:
            ...
            def _wc(since, *extra, _tag=tag):
                return db.query(WarAttack).filter(
                    WarAttack.attacker_tag == _tag,
                    WarAttack.war_type == "war",
    ...
    # Sort linked rows: hit rate desc (None last), then TH desc
    ...
    return render_template("coc_members.html", user=session["user"], guild=guild,
                           rows=rows, no_account_rows=no_account_rows,
